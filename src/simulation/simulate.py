...
Purpose: Simulates realistic sequencing by stepping the ODE forward in time,
            generating tagged output, adding noise, and computing efficiency.
...
# time definition - since PLCs and SCADA work in discrete time; and since
# 1-second resolution is realistic
dt = 1.0        # seconds
t_end = 3600    # 1 hour
time = np.arange(0, t_end, dt)  # dt = step value in interval (0,t_end)
...
for k in range(len(time)):
    if (Q_in[k] > 0):
        Efficiency[k] = (
            (params["m_dot"] * params["c_p"] * (Temp_Out[k] - params["T_in"]))
            / Q_in[k]
        )
    else:
        Efficiency[k] = 0.0
